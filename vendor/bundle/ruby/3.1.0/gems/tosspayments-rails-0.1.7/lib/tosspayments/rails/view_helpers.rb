  ...
  module Rails
    module ViewHelpers
      ...
      def tosspayments_format_amount(amount, currency: 'KRW', suffix: '원')
        num = amount.to_i
        formatted = num.to_s.gsub(/\B(?=(\d{3})+(?!\d))/, ',')
      ...
      # 결제 요청 함수 생성
      def tosspayments_payment_script(order_id:, order_name:, success_url:, fail_url:, amount: _amount_unused = nil,
                                      options: {})
        script_content = <<~JAVASCRIPT
      ...
      # 토스페이먼츠 브랜드페이 스크립트
      def tosspayments_brandpay_script(customer_key:, redirect_url:, options: _options_unused = {})
        client_key = Tosspayments::Rails.configuration.client_key
      ...
      # 결제 폼을 위한 기본 HTML 구조
      def tosspayments_payment_form(order_id:, order_name:, amount: _amount_unused = nil, options: {})
        payment_id = (options[:payment_selector] || '#payment-widget').delete('#')
        agreement_id = (options[:agreement_selector] || '#agreement').delete('#')
        customer_key = options[:customer_key] || 'ANONYMOUS'
        client_key = Tosspayments::Rails.configuration.client_key
        success_url = begin
          options[:success_url] || main_app.url_for(controller: :payments, action: :success,
                                                    only_path: false)
        rescue StandardError
          '/payments/success'
        end
        fail_url = begin
          options[:fail_url] || main_app.url_for(controller: :payments, action: :fail,
                                                 only_path: false)
        rescue StandardError
          '/payments/fail'
        end
        wrapper_classes = ['tosspayments-payment-wrapper']

        content_tag :div,
                    class: wrapper_classes.join(' '),
                    data: {
                      controller: 'tosspayments',
                      'tosspayments-client-key-value': client_key,
                      'tosspayments-customer-key-value': customer_key,
                      'tosspayments-order-id-value': order_id,
                      'tosspayments-order-name-value': order_name,
                      'tosspayments-success-url-value': success_url,
                      'tosspayments-fail-url-value': fail_url
                    } do
          concat content_tag(:div, '', id: payment_id, data: { role: 'payment-widget' })
          concat content_tag(:div, '', id: agreement_id, data: { role: 'agreement' })
          concat content_tag(:button, '결제하기',
                             class: 'btn btn-primary tosspayments-payment-button',
                             data: { action: 'click->tosspayments#requestPayment', 'tosspayments-target': 'payButton' })
        end
      end
    end
  end
